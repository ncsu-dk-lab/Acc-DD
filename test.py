...
def load_data_path(args):
    ...
    if args.dataset == 'imagenet':
        ...
        if 'idc' in args.slct_type:     # todo
            ...
            # data, target = torch.load(os.path.join(f'{args.save_dir}', 'data.pt'))
            save_dir_here = 'results/cifar10/conv3in_grad_mse_inloop100_cut_niter2000_factor2_lr0.02_mix_ipc10_pretrainweight-perturbation-large-2_mnum5/data_best.pt'
...
def test_data(args,
    ...
    for model_fn in model_fn_ls:
        ...
        for _ in range(repeat):
            ...
            torch.save(model.state_dict(), 'cifar-10-syn-convnet.pt')
            best_acc_l.append(best_acc)
            acc_l.append(acc)
        logger(
            f'Repeat {repeat} => Best, last acc, std: {np.mean(best_acc_l):.1f} {np.mean(acc_l):.1f} {np.std(best_acc_l):.1f}\n')
...
if __name__ == '__main__':
    ...
    if args.dataset[:5] == 'cifar':
        ...
        test_data(args, train_loader, val_loader, repeat=args.repeat, model_fn=convnet)
